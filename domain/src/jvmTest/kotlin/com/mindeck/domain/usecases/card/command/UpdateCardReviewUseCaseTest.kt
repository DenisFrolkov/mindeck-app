...
class UpdateCardReviewUseCaseTest {
    ...
    @Test
    fun `NEW + AGAIN transitions card to LEARNING with reduced easeFactor`() =
        runTest {
            val card = newCard()
            setupRepository()
    ...
    @Test
    fun `NEW + HARD transitions card to LEARNING with reduced easeFactor`() =
        runTest {
            val card = newCard()
            setupRepository()
    ...
    @Test
    fun `NEW + GOOD advances learning step to 1`() =
        runTest {
            val card = newCard()
            setupRepository()
    ...
    @Test
    fun `NEW + GOOD on last learning step graduates card to REVIEW`() =
        runTest {
    ...
    @Test
    fun `NEW + EASY immediately graduates card to REVIEW with interval 4`() =
        runTest {
            val card = newCard()
            setupRepository()
    ...
    @Test
    fun `REVIEW + HARD multiplies interval by 120 percent and reduces easeFactor`() =
        runTest {
    ...
    @Test
    fun `REVIEW + GOOD multiplies interval by easeFactor`() =
        runTest {
            val card = newCard().copy(cardState = CardState.REVIEW, interval = 5f)
            setupRepository()
    ...
    @Test
    fun `easeFactor does not drop below MIN_EASE_FACTOR on AGAIN`() =
        runTest {
    ...
    @Test
    fun `easeFactor does not exceed MAX_EASE_FACTOR on EASY`() =
        runTest {
    ...
    @Test
    fun `LAPSE + AGAIN keeps card in LAPSE state`() =
        runTest {
    ...
    @Test
    fun `LAPSE + GOOD graduates to REVIEW with interval coerced to minimum 1f`() =
        runTest {
    ...
    @Test
    fun `REVIEW + AGAIN transitions to LAPSE and increments lapseCount`() =
        runTest {
    ...
    @Test
    fun `lastReviewDate is updated to current time on each review`() =
        runTest {
