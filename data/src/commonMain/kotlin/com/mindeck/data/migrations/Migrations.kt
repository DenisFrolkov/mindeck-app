package com.mindeck.data.migrations
...
val MIGRATION_1_2 =
    object : Migration(1, 2) {
        override fun migrate(connection: SQLiteConnection) {
            ...
            connection.execSQL("DROP TABLE `deck`")
            connection.execSQL("ALTER TABLE `deck_new` RENAME TO `deck`")
            connection.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_deck_deck_name` ON `deck` (`deck_name`)")
...
val MIGRATION_4_5 =
    object : Migration(4, 5) {
        override fun migrate(connection: SQLiteConnection) {
            connection.execSQL("CREATE INDEX IF NOT EXISTS `index_card_card_state` ON `card` (`card_state`)")
            connection.execSQL("CREATE INDEX IF NOT EXISTS `index_card_next_review_date` ON `card` (`next_review_date`)")
            connection.execSQL("CREATE INDEX IF NOT EXISTS `index_card_first_review_date` ON `card` (`first_review_date`)")
        }
    }
