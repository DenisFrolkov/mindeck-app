...
// Migration v5 → v6: add card_image_path column for storing image file path.
val MIGRATION_5_6 =
    object : Migration(5, 6) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `card` ADD COLUMN `card_image_path` TEXT")
        }
    }
...
val MIGRATION_6_7 =
    object : Migration(6, 7) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `card` ADD COLUMN `card_question_audio_path` TEXT")
            db.execSQL("ALTER TABLE `card` ADD COLUMN `card_answer_audio_path` TEXT")
        }
    }
